...
$admin_username = 'admin'; // Đặt tên đăng nhập admin ở đây
$admin_password = 'admin123'; // Đặt mật khẩu admin ở đây
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["signin"])) {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        if ($username === $admin_username && $password === $admin_password) {
            // Lưu thông tin đăng nhập admin vào Session
            $_SESSION['user_admin'] = $username;
            header("Location: admin/index.php");
            exit();
        } else {
            // Kiểm tra thông tin đăng nhập user từ cơ sở dữ liệu (giả sử bạn có một hàm get_user_by_username)
